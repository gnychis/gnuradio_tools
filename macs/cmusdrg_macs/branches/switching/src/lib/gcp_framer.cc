...
// here.
void gcp::framer(const std::vector<unsigned char> input, pmt_t demod_properties)
{
  int bit=0;
...
void gcp::framer_calculate_timestamp(unsigned long timestamp, int bit, int nbits, long sps, long bps)
{
...
// layer into a buffer.
void gcp::framer_new_header_bit(unsigned char bit)
{
...
void gcp::framer_new_payload_bit(unsigned char bit)
{
  d_payload_bits.push_back(bit);

  if((int)d_payload_bits.size() == (int)(d_cframe_hdr.payload_len * 8)) {
...
if(verbose)
  ...
  if(!((int)d_cframe_hdr.payload_len>0) || !((int)d_cframe_hdr.payload_len <= (max_frame_payload()))) {
    if(verbose)
...
void gcp::framer_have_frame(pmt_t uvec)
{
  ...
  if((int)d_cframe_hdr.crc == (int)bcrc.checksum())
...
void gcp::build_frame(pmt_t data)
{
